...
public class BuyChair : Shopping
{
    ...
    public override void Buy(ShopSO shopSO)
    {
        if (placeWithChairs.capacity < placeWithChairs.maxCapacity)
        {
            ...
            {
                Vector3 pos = new Vector3(3 * Mathf.Cos(j * Mathf.PI / 180) + placeWithChairs.gameObject.transform.position.x, placeWithChairs.gameObject.transform.position.y, 3 * Mathf.Sin(j * Mathf.PI / 180) + placeWithChairs.gameObject.transform.position.z);
